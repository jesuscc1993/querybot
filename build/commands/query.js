...
exports.query = function (discordBot, message, input, parameters, metadata) {
  var commandCount = message.content.split('\n').reduce(function (count, line) {
    return count + (lineContainsPrefix(line, settings_1.botPrefixDefault) ? 1 : 0);
  }, 0);
  if (commandCount > 1) {
  ...
  if ((!keywordSearch || keyword) && parameters.length) {
    ...
    if (guild_1) {
      providers_1.ServerProvider.getInstance()
        ...
        .pipe(
          operators_1.tap(function (searchResultItems) {
          ...
          operators_1.catchError(function (error) {
            if (error === providers_1.invalidKeywordError) return rxjs_1.of();
